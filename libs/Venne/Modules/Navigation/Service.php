...
class Service extends Venne\Developer\Service\DoctrineService {
	...
	public $entityNamespace = "\\Venne\\NavigationModule\\";
	...
	public function hookAdminMenu($menu)
	{
		...
		$nav->addKey("module", "Admin:Navigation");
		$menu[] = $nav;
	}
	...
	public function getCurrentList($without = Null, $layer = 0, $depend = Null)
	{
		...
		$text = "";
		if (!$depend)
			$menu = $em->createQuery('SELECT u FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent IS NULL')
					->getResult();
		else
			$menu = $em->createQuery('SELECT u FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent= :depend ')
	...
	public function getList($without = Null, $layer = 0, $depend = Null)
	{
		...
		$text = "";
		if (!$depend)
			$menu = $em->createQuery('SELECT u FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent IS NULL')->getResult();
		else
			$menu = $em->createQuery('SELECT u FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent= :depend ')
					->setParameters(array("depend" => $depend))
					->getResult();
	...
	{
		if ($parent_id) {
			$query = $this->getEntityManager()->createQuery('SELECT MAX(u.order) FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent = ?2')->setParameter(2, $parent_id);
		} else {
			$query = $this->getEntityManager()->createQuery('SELECT MAX(u.order) FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent is NULL');
		}
	...
	public function updateModuleItem(\Venne\NavigationModule\NavigationEntity $menuEntity, $moduleName, $moduleItemId, $name, $parent_id, $paramsArray, $withoutFlush = false)
	{
	...
	{
		$query = $this->getEntityManager()->createQuery('SELECT u FROM \Venne\NavigationModule\NavigationEntity u WHERE u.parent = ?1 AND u.order > ?2')->setParameter(1, isset($entity->parent->id) ? $entity->parent->id : NULL)->setParameter(2, $entity->order);
