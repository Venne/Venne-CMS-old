...
class Service extends Venne\Developer\Service\DoctrineService {
	...
	public function getCurrentLang(\Nette\Http\Request $httpRequest)
	{
		if (!$this->currentLang) {
			...
			$website = $this->getContainer()->cms->website->current;
			...
			} else if ($website->langType == \Venne\Modules\Website::LANG_PARSE_URL) {
			...
			} else if ($website->langType == \Venne\Modules\Website::LANG_IN_GET) {
	...
	public function getCurrentFrontLang(\Nette\Http\Request $httpRequest)
	{
		if (!$this->currentFrontLang) {
			$repo = $this->getRepository();
			$website = $this->getContainer()->cms->website->currentFront;
