...
class LayoutModel extends Venne\CMS\Developer\Model {
	...
	{
		$presenter = $this->container->application->presenter;
		$name = $presenter->getName() . ":" . $presenter->getAction();
		$data = array();


		$query = $this->getEntityManager()->createQuery('
					SELECT u FROM \Venne\CMS\Modules\Layout u WHERE u.website = ?1 ORDER BY u.regex DESC
				')->setParameter(1, $this->container->website->current);
	...
	public function saveLayout($moduleItemId, $moduleName, $use, $layout, $linkParams)
	{
		...
		if (!$use) {
			...
			if (!$entity) {
				...
				$entity->regex = "";
				$entity->website = $this->container->website->currentFront;
	...
	public function createLayout($layoutName, $module = NULL, $presenter = NULL, $action = NULL, $params = NULL, $moduleName = NULL, $moduleItemId = NULL)
	{
		...
		$layout->regex = "";
		$layout->website = $this->container->website->currentFront;
