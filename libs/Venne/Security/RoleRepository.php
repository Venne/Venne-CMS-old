...
class RoleRepository extends BaseRepository {
	...
	public function getList($without = Null, $layer = 0, $depend = Null)
	{
		...
		$text = "";
		if (!$depend)
			$menu = $em->createQuery('SELECT u FROM \Venne\CMS\Modules\Role u WHERE u.parent IS NULL')
					->getResult();
		else
			$menu = $em->createQuery('SELECT u FROM \Venne\CMS\Modules\Role u WHERE u.parent= :depend ')
	...
	public function setStructure($data)
	{
		foreach ($data as $item) {
			foreach ($item as $item2) {
				...
				$entity->parent = $this->find($item2["role_id"]);
			}
		}
