...
namespace AdminModule\NavigationModule;

use Nette\Utils\Html;

/**
 * @resource AdminModule\NavigationModule
 */
class DefaultPresenter extends \Venne\Developer\Presenter\AdminPresenter {
	...
	public function startup()
	{
		...
		$this->addPath("Navigation", $this->link(":Admin:Navigation:Default:"));
		$this->template->menu = $this->context->services->navigation->getRootItems();
	...
	{
		$this->addPath("new item", $this->link(":Admin:Navigation:Default:create"));
	}
	...
	{
		$this->addPath("edit" . " (" . $this->id . ")", $this->link(":Admin:Navigation:Default:edit"));
	}
	...
	public function formSaveRecursion($form, $menu)
	{
		foreach ($menu as $key => $item) {
			if ($form["delete_" . $item->id]->isSubmittedBy()) {
				$this->context->services->navigation->delete($this->context->services->navigation->repository->find($item->id));
	...
	public function handleSortSave()
	{
		...
		$this->context->services->navigation->setStructure($data);
	...
	{
		$form = new \Venne\NavigationModule\NavigationForm($this, $name);
