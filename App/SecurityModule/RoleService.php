...
class RoleService extends \Venne\Developer\Service\DoctrineService {
	...
	public function getList($without = Null, $layer = 0, $depend = Null)
	{
		...
		$text = "";
		if (!$depend)
			$menu = $em->createQuery('SELECT u FROM \App\SecurityModule\RoleEntity u WHERE u.parent IS NULL')
					->getResult();
		else
			$menu = $em->createQuery('SELECT u FROM \App\SecurityModule\RoleEntity u WHERE u.parent= :depend ')
